...
def foodHeuristic(state, problem):
    ...
    """
    position, foodGrid = state  # the state is a tuple (pacmanPosition, foodGrid)
    h_bfs_max = 0

    for foodposition in foodGrid.asList():
        # Use the given mazeDintance to calculate the maximum distance from the current location to all beans from the BFS search
        h_bfs_max = max(h_bfs_max, mazeDistance(foodposition, position, problem.startingGameState))
    return h_bfs_max
