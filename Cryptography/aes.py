class AES:
    ...
    def check_pt(self):
        ...
        while 1:
            ...
            plaintext = input()
            plen = len(plaintext)
            if plen != 0 and plen % 16 == 0:
                print("你输入的明文为: ", plaintext)
                return plaintext
            else:
                print("明文字符长度必须为16的倍数,现在的长度为", plen)
    ...
    # 加密过程
    def encryption(self, plaintext, round_key):
        print("\n------encryption------")
        plaintext_list = []
        ciphertext_list = []
        plen = len(plaintext)
        ...
        for pt in plaintext_list:
            ...
            ct = self.aes(pt, round_key)             # 加密的过程
    ...
    def decryption(self, round_key):
        ...
        for ct in ciphertext_list:
            ...
            pt = self.normalization(pt)                      # list -> string
...
if __name__ == '__main__':
    ...
    # 进行加密
    aes.encryption(plaintext, round_key)

    # 进行解密
    aes.decryption(round_key)
